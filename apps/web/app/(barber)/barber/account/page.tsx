...
async function AccountContent() {
  ...
  const phone = phoneDigits.startsWith("60")
    ...
    : phoneDigits.startsWith("0")
      ? phoneDigits.slice(1)
      : phoneDigits;
  const initials = buildInitials(fullName || email || "Barber");
  ...
  const saveProfile = async (formData: FormData) => {
    ...
    const newFirstName = String(formData.get("first_name") ?? "").trim();
    const newLastName = String(formData.get("last_name") ?? "").trim();
    const rawPhoneInput = String(formData.get("phone") ?? "").trim().replace(/\D/g, "");
    const newPhone = rawPhoneInput ? `+60${rawPhoneInput}` : null;
          ...
          <form action={saveProfile} className="space-y-6">
            <div className="flex items-center gap-4">
              <Avatar className="h-12 w-12">
                <AvatarImage src={avatarUrl} alt={fullName || "Barber"} />
