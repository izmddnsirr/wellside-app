...
export async function getAvailableSlots(
  ...
  const { data: profile, error: profileError } = await (supabase as {
    from: (table: string) => {
        ...
        eq: (column: string, value: unknown) => {
          neq: (column: string, value: unknown) => {
            lt: (column: string, value: unknown) => {
              gt: (column: string, value: unknown) => Promise<{
  ...
  const { data: unavailabilityRows, error: unavailabilityError } = await (
    supabase as {
      from: (table: string) => {
        select: (columns: string) => {
          eq: (column: string, value: unknown) => {
